...
- `corpus_md/` — **규약 형식 통일 코퍼스** (정본: 감사기준 39 + 회계기준 63 = 102개 파일 9,885문단 / **BC·IE 갈래**: `kifrs_<번호>_bc.md`·`_ie.md` 101개 파일 10,908문단 — K-IFRS 결론도출근거·적용사례 + 재무보고 개념체계 BC, 규약 4.3-5). `scripts/normalize_corpus.py`가 아래 두 원본 폴더에서 생성하며 재실행으로 전체 재생성 가능. 파일명 `ksa_<번호>.md`/`kifrs_<번호>.md`. 수록 범위·원본 결함 보정 내역은 `corpus_md/README.md` 참조 (감사기준 예시문은 `부록N`/`부록-사례N` 가상 번호로 분리, 부록 A 용어정의는 `정의-<용어>` 행 단위 분리. BC·IE는 검색 옵트인 — include_bc/include_examples. 개념체계류 CF·MC·PS2도 기본 제외 — include_framework).
